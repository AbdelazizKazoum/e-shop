...
const TabFilters = ({
  ...
  const {
    ...
    brands: storeBrands, // <-- Add brands from store
    sizes: storeSizes,
  ...
  const [localBrands, setLocalBrands] = useState(storeBrands); // <-- Add localBrands
  ...
  useEffect(() => {
    ...
    setLocalBrands(storeBrands); // <-- Sync brands
  ...
  const handleApplyFilters = () => {
    ...
    setBrands(localBrands); // <-- Apply brands
    setSizes(localSizes);
        ...
        <>
          ...
          >
            ...
            {!storeBrands.length ? (
              ...
              <span onClick={() => setBrands([])}>{renderXClear()}</span>
            )}
          </Popover.Button>
          <Transition
            as={Fragment}
                  ...
                  {brands &&
                    brands.map((item) => (
                      <Checkbox
                        ...
                        name={item.name}
                        label={item.name}
                        defaultChecked={localBrands.includes(item.name)}
                        onChange={(checked) =>
  ...
  return (
    <div className="flex lg:space-x-4">
      <div className="hidden lg:flex flex-1 space-x-4 items-center">
        ...
        {renderTabsBrands()} {/* <-- Add brands tab */}
        {renderTabsSize()}
        {renderTabsPriceRage()}
        {renderTabIsOnsale()}
